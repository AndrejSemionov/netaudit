...
[![Bitcoin](https://img.shields.io/badge/Bitcoin-BTC-f7931a?logo=bitcoin&logoColor=white)](#поддержать-проект)
[![Zcash](https://img.shields.io/badge/Zcash-ZEC-ecb244?logo=zcash&logoColor=white)](#поддержать-проект)
...
при этом содержимое логов и не детектируя ничего security-значимого. Специально работает без
sudo: показывает, что видно текущему уровню SSH-доступа NetAudit, и отдельно помечает, какие
источники требуют повышенных прав для чтения. Полезен сам по себе перед запуском любого из
двух аудитов ниже — чтобы понять, что вообще реально доступно на конкретном хосте.

```bash
python3 netaudit.py run log_discovery --host 1.2.3.4 --user root
```

**Nginx Logs Audit** (`nginx_logs_audit`, SSH) — резолвит директивы `access_log`/`error_log`
каждого блока `server {}` (включая правила каскадного наследования nginx), сопоставляет их с
тем, что реально обнаруживается на диске, дедуплицирует повторяющиеся источники между
блоками, и анализирует собранное содержимое на: аномалии частоты запросов, сканирование путей
(повторяющиеся 404 — признак перебора), всплески запросов, высокую долю ошибок парсинга
(несовпадение формата лога), высокую частоту ошибок, критичные записи в error-логе и
повторяющиеся одинаковые сообщения об ошибках. Несколько блоков `server {}`, указывающих на
несколько разных файлов логов, объединяются в один общий анализ, а не обрабатываются по
отдельности на каждый блок.

```bash
python3 netaudit.py run nginx_logs_audit --host 1.2.3.4 --user root
```

Покрытие (`coverage`) явно указывается по каждому источнику
(`complete`/`partial`/`empty`/`failed`/`unknown`), а не молча приравнивает «успешно собрали
ноль событий» к «сбор не удался». Результат `empty` с `detection_succeeded: true` означает,
что проверка отработала корректно, и в собранном окне действительно ничего не было — это
принципиально другая ситуация, чем ошибка сбора.

**SSH Authentication Audit** (`ssh_auth_audit`, SSH) — анализирует активность SSH-аутентификации
(`auth.log`, с откатом на журнал systemd, если `auth.log` недоступен или нечитаем) на предмет
повторяющихся неудачных попыток входа, перебора несуществующих пользователей, распределённых
попыток подбора (много source-IP, мало попыток с каждого) и успешных входов, последовавших
за серией неудач с того же источника. `auth.log` и журнал рассматриваются как альтернативы —
используется то, что реально читаемо, они никогда не объединяются в один результат.
...
- SSH-проверки по умолчанию read-only. Операции, изменяющие целевую систему — автоустановка
  недостающего инструмента (`lynis_audit`, `rootkit_check`, `aide_check`) или переинициализация
  эталонной базы AIDE (`aide_check` с `mode=init`) — требуют явного подтверждения
  («yes — modify the target system») перед запуском, и в CLI, и в веб-интерфейсе. Без него
  проверка сообщает, что бы она сделала, и останавливается.
- API-ключ Anthropic берётся из переменной окружения / локальной БД, не хранится в коде/отчётах.
