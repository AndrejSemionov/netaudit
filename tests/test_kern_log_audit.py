"""Tests for netaudit_pkg.checks.kern_log_audit — the @register
orchestration wiring Discovery -> Collection -> Parser -> Detection ->
Findings for Kern Log Audit.

Scope, deliberately (mirrors test_fail2ban_logs_audit.py's own scope
discipline): this file tests only the NEW logic this module adds —
    ...
    2. The registry entrypoint's own contract (no-host, connection
       failure, host key mismatch, closes SSH on error) — same shape as
       test_fail2ban_logs_audit.py's / test_nginx_logs_audit.py's
       equivalent tests, since check_kern_log_audit() is a thin
       SSH-connect wrapper with the identical contract.
    3. A regression guard confirming checks/__init__.py actually imports
       this module — without the import, @register never runs.

A full mocked-SSH E2E (Discovery's probe, Collection's tail, Parser's
line-by-line dispatch, Detection's two signal types, Findings' severity
mapping) is deliberately NOT re-tested here — every one of those layers
already has its own test suite (test_kern_log_parser.py,
test_kern_log_detection.py, test_kern_log_findings.py). Re-asserting
...
from netaudit_pkg.checks.kern_log_audit import (
    _source_coverage,
    check_kern_log_audit,
)
from netaudit_pkg.kern_log_detection import CoverageStatus
...
def _collected(*, completed=True, exit_code=0, line_count=0, stdout=''):
    cmd_result = CommandResult(completed=completed, exit_code=exit_code, stdout=stdout, stderr='',
                                command='tail -n 200 /var/log/kern.log')
    return CollectionResult(source_kind=SourceKind.FILE, source_path='/var/log/kern.log',
...
# ===========================================================================
# Registry entrypoint — mirrors test_fail2ban_logs_audit.py's equivalent tests
# ===========================================================================

def test_check_kern_log_audit_no_host():
    result = check_kern_log_audit(host='')
    assert result == {'error': 'host not specified'}


def test_check_kern_log_audit_connection_failure(monkeypatch):
    class _BoomSSH:
    ...
    monkeypatch.setattr('netaudit_pkg.checks.kern_log_audit.SSHExecutor', _BoomSSH)
    result = check_kern_log_audit(host='10.0.0.5')
    assert 'error' in result
    assert 'could not connect' in result['error']


def test_check_kern_log_audit_host_key_mismatch(monkeypatch):
    class _MismatchSSH:
        def __init__(self, *a, **kw):
            pass
    ...
    monkeypatch.setattr('netaudit_pkg.checks.kern_log_audit.SSHExecutor', _MismatchSSH)
    result = check_kern_log_audit(host='10.0.0.5')
    assert result == {'error': 'host key changed'}


def test_check_kern_log_audit_closes_ssh_even_on_error(monkeypatch):
    ...
    monkeypatch.setattr('netaudit_pkg.checks.kern_log_audit.SSHExecutor', lambda *a, **kw: fake)
    check_kern_log_audit(host='10.0.0.5')
...
def test_kern_log_audit_is_imported_by_checks_package():
    import netaudit_pkg.checks as checks_pkg
    assert hasattr(checks_pkg, 'kern_log_audit')
